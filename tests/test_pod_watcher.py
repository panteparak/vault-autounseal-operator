...
class TestPodWatcher:
    ...
    def mock_k8s_client(self):
        with patch('vault_autounseal_operator.pod_watcher.client.CoreV1Api') as mock_client_class:
            mock_client = Mock()
    ...
    def test_pod_watcher_initialization(self, mock_vault_client):
        ...
        assert watcher.pod_selector == {"matchLabels": {"app": "vault", "role": "server"}}
    ...
    @pytest.mark.asyncio
    async def test_is_vault_pod_ready_and_sealed_running_and_sealed(self, pod_watcher, mock_vault_client):
    ...
    @pytest.mark.asyncio
    async def test_is_vault_pod_ready_and_sealed_not_running(self, pod_watcher, mock_vault_client):
        pod = {
            'status': {
                'phase': 'Pending',
                'podIP': '10.0.0.1',
    ...
    @pytest.mark.asyncio
    async def test_is_vault_pod_ready_and_sealed_no_ip(self, pod_watcher, mock_vault_client):
        pod = {
            'status': {
                'phase': 'Running',
                'containerStatuses': [{'ready': True}]
    ...
    @pytest.mark.asyncio
    async def test_is_vault_pod_ready_and_sealed_containers_not_ready(self, pod_watcher, mock_vault_client):
        pod = {
            'status': {
                'phase': 'Running',
                'podIP': '10.0.0.1',
    ...
    @pytest.mark.asyncio
    async def test_is_vault_pod_ready_and_sealed_unsealed(self, pod_watcher, mock_vault_client):
    ...
    @pytest.mark.asyncio
    async def test_is_vault_pod_ready_and_sealed_vault_error(self, pod_watcher, mock_vault_client):
    ...
    async def test_attempt_unseal_success(self, pod_watcher, mock_vault_client):
        ...
        mock_vault_client.unseal.assert_called_once_with(
            ["key1", "key2", "key3"], 2
        )
    ...
    @pytest.mark.asyncio
    async def test_handle_pod_event_added_sealed_pod(self, pod_watcher, mock_vault_client):
        mock_vault_client.is_sealed.return_value = True
        mock_vault_client.unseal.return_value = {'sealed': False}
    ...
    @pytest.mark.asyncio
    async def test_handle_pod_event_added_unsealed_pod(self, pod_watcher, mock_vault_client):
        mock_vault_client.is_sealed.return_value = False
    ...
    @pytest.mark.asyncio
    async def test_handle_pod_event_modified_now_unsealed(self, pod_watcher, mock_vault_client):
        # Add a pod to monitored pods first
        pod_watcher.monitored_pods['uid-123'] = {
            'name': 'vault-0',
            'namespace': 'vault',
    ...
    @pytest.mark.asyncio
    async def test_handle_pod_event_wrong_selector(self, pod_watcher, mock_vault_client):
        pod = {
            'metadata': {
                'name': 'redis-0',
                'uid': 'uid-123',
    ...
    async def test_start_and_stop(self, pod_watcher):
        ...
        with patch.object(pod_watcher, '_watch_pods', new_callable=AsyncMock) as mock_watch:
            # Start watcher
    ...
    async def test_watch_pods_api_error_410(self, pod_watcher, mock_k8s_client):
        ...
        with patch('vault_autounseal_operator.pod_watcher.watch.Watch') as mock_watch_class:
            mock_watch = Mock()
    ...
    async def test_watch_pods_unexpected_error(self, pod_watcher, mock_k8s_client):
        ...
        with patch('vault_autounseal_operator.pod_watcher.watch.Watch') as mock_watch_class:
            ...
            with patch('vault_autounseal_operator.pod_watcher.asyncio.sleep', new_callable=AsyncMock) as mock_sleep:
