...
class TestIntegration:
    ...
    @pytest.mark.asyncio
    async def test_end_to_end_vault_unseal_with_direct_keys(self, operator, mock_vault_responses):
        """Test complete flow: spec validation -> vault client setup -> unsealing"""
        
        # Setup mocks
        with patch('vault_autounseal_operator.operator_v2.VaultClient') as mock_vault_client_class:
            mock_vault_client = Mock()
            mock_vault_client.is_sealed = AsyncMock(return_value=True)
            mock_vault_client.unseal = AsyncMock(return_value=mock_vault_responses['unsealed_status'])
            ...
            # Check and unseal
            vault_status = await operator.check_and_unseal_vault(validated_spec, 'default', 'test-vault')
    ...
    @pytest.mark.asyncio
    async def test_end_to_end_vault_unseal_with_secret_ref(self, operator, mock_k8s_secret, mock_vault_responses):
        """Test complete flow using Kubernetes secret reference"""
        
        with patch('vault_autounseal_operator.operator_v2.VaultClient') as mock_vault_client_class, \
             patch('vault_autounseal_operator.operator_v2.client.CoreV1Api') as mock_k8s_client_class:
    ...
    async def test_error_handling_and_recovery(self, operator, mock_vault_responses):
        ...
        with patch('vault_autounseal_operator.operator_v2.VaultClient') as mock_vault_client_class:
            ...
            mock_vault_client.is_sealed = AsyncMock(side_effect=Exception("Connection refused"))
            ...
            mock_vault_client.is_sealed = AsyncMock(return_value=True)
            mock_vault_client.unseal = AsyncMock(side_effect=[
    ...
    async def test_ha_mode_pod_watching_setup(self, operator):
        ...
        with patch('vault_autounseal_operator.operator_v2.VaultClient') as mock_vault_client_class, \
             patch('vault_autounseal_operator.operator_v2.PodWatcher') as mock_pod_watcher_class:
    ...
    async def test_concurrent_operations_thread_safety(self, operator):
        ...
        with patch('vault_autounseal_operator.operator_v2.VaultClient') as mock_vault_client_class:
            ...
            async def setup_and_check(name):
                ...
                return await operator.check_and_unseal_vault(validated_spec, 'default', name)
    ...
    async def test_memory_management_sensitive_data(self, operator):
        ...
        with patch('vault_autounseal_operator.operator_v2.VaultClient') as mock_vault_client_class:
            ...
            validated_spec = SecurityValidator.validate_spec(spec)
            await operator.setup_vault_client(validated_spec, 'default', 'test-vault')
            await operator.check_and_unseal_vault(validated_spec, 'default', 'test-vault')
    ...
    def test_input_sanitization_prevents_log_injection(self):
        ...
        assert sanitized['keys'] == ['key1\nFAKE_LOG_ENTRY', 'key2']  # Not a sensitive field name