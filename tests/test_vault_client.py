...
class TestVaultClient:
    ...
    async def test_is_sealed_exception(self, vault_client, mock_hvac_client):
        mock_hvac_client.sys.read_seal_status.side_effect = Exception(
            "Connection error"
        )
    ...
    async def test_unseal_invalid_inputs(self, vault_client):
        ...
        with pytest.raises(
            ValueError, match="Threshold exceeds number of available keys"
        ):
    ...
    def test_security_headers_configured(self, mock_hvac_client):
        with patch(
            "vault_autounseal_operator.vault_client.requests.Session"
    ...
    def test_retry_strategy_configured(self, mock_hvac_client):
        with (
            ...
            patch(
                "vault_autounseal_operator.vault_client.HTTPAdapter"
