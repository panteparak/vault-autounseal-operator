...
class TestSecurityValidator:
    ...
    def test_validate_url_valid(self):
        # Valid URLs
        assert (
            SecurityValidator.validate_url("https://vault.example.com:8200")
            == "https://vault.example.com:8200"
        )
        assert (
            SecurityValidator.validate_url("http://localhost:8200")
            == "http://localhost:8200"
        )
        assert (
            SecurityValidator.validate_url("https://vault.example.com/v1")
    ...
    def test_sanitize_log_data(self):
        ...
        data = {
            ...
            "metadata": {"token": "secret-token", "name": "public-name"},
        }
    ...
    def test_validate_spec_invalid(self):
        ...
        spec = {
            "url": "https://vault.example.com",
            "unsealKeys": {"secret": ["key"], "secretRef": {"name": "secret"}},
        }
        with pytest.raises(ValueError, match="Cannot specify both"):
            SecurityValidator.validate_spec(spec)

        # Neither secret nor secretRef specified
        spec = {"url": "https://vault.example.com", "unsealKeys": {}}
