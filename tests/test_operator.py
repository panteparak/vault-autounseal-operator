...
from vault_autounseal_operator.operator_v2 import VaultOperatorV2, create_vault_config, update_vault_config, delete_vault_config
...
class TestVaultOperatorV2:
    ...
    def mock_k8s_client(self, operator):
        with patch('vault_autounseal_operator.operator_v2.client.CoreV1Api') as mock_client_class:
            mock_client = Mock()
    ...
    def mock_vault_client(self):
        with patch('vault_autounseal_operator.operator_v2.VaultClient') as mock_client_class:
            mock_client = Mock()
            mock_client_class.return_value = mock_client
            yield mock_client
    ...
    def mock_pod_watcher(self):
        with patch('vault_autounseal_operator.operator_v2.PodWatcher') as mock_watcher_class:
            mock_watcher = Mock()
    ...
    @pytest.mark.asyncio
    async def test_get_unseal_keys_secret_ref_newline_separated(self, operator, mock_k8s_client):
    ...
    @pytest.mark.asyncio
    async def test_get_unseal_keys_missing_key_in_secret(self, operator, mock_k8s_client):
        mock_secret = Mock()
    ...
    @pytest.mark.asyncio
    async def test_setup_vault_client_basic(self, operator, mock_vault_client, mock_pod_watcher):
        spec = {
    ...
    @pytest.mark.asyncio
    async def test_setup_vault_client_with_ha(self, operator, mock_vault_client, mock_pod_watcher, mock_k8s_client):
        spec = {
            'url': 'https://vault.example.com:8200',
    ...
    @pytest.mark.asyncio
    async def test_cleanup_vault_instance(self, operator, mock_vault_client, mock_pod_watcher):
    ...
    @pytest.mark.asyncio
    async def test_check_and_unseal_vault_already_unsealed(self, operator, mock_vault_client, mock_k8s_client):
    ...
    @pytest.mark.asyncio
    async def test_check_and_unseal_vault_sealed(self, operator, mock_vault_client, mock_k8s_client):
        # Mock vault client methods
    ...
    @pytest.mark.asyncio
    async def test_check_and_unseal_vault_error(self, operator, mock_vault_client, mock_k8s_client):
        # Mock vault client to raise exception
        mock_vault_client.is_sealed = AsyncMock(side_effect=Exception("Connection error"))
...
class TestOperatorHandlers:
    ...
    def mock_security_validator(self):
        with patch('vault_autounseal_operator.operator_v2.SecurityValidator') as mock_validator:
            ...
            mock_validator.sanitize_log_data.return_value = {'url': 'https://vault.example.com:8200'}
            yield mock_validator
    
    @pytest.mark.asyncio
    async def test_create_vault_config_success(self, mock_operator, mock_security_validator):
    ...
    @pytest.mark.asyncio
    async def test_create_vault_config_validation_error(self, mock_operator, mock_security_validator):
    ...
    @pytest.mark.asyncio
    async def test_create_vault_config_vault_error(self, mock_operator, mock_security_validator):
    ...
    async def test_update_vault_config(self, mock_operator, mock_security_validator):
        mock_vault_status = VaultStatus(sealed=False, lastUnsealed='2023-01-01T00:00:00Z', lastChecked='2023-01-01T00:00:00Z', error=None)
        mock_operator.check_and_unseal_vault.return_value = mock_vault_status
        
        body = {'spec': {'url': 'https://vault.example.com:8200', 'unsealKeys': {'secret': ['key1']}}}
        ...
        mock_operator.cleanup_vault_instance.assert_called_once_with('default', 'test-vault')
        mock_operator.setup_vault_client.assert_called_once()
