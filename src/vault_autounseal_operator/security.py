...
class SecurityValidator:
    ...
    def validate_url(cls, url: str) -> str:
        ...
        try:
            ...
            if parsed.scheme.lower() not in cls.ALLOWED_SCHEMES:
                raise ValueError(
                    f"URL scheme must be one of: {', '.join(cls.ALLOWED_SCHEMES)}"
                )
    ...
    def validate_kubernetes_name(cls, name: str, field_name: str = "name") -> str:
        ...
        max_length = (
            cls.MAX_SECRET_NAME_LENGTH
            if field_name == "secret name"
            else cls.MAX_NAMESPACE_LENGTH
        )
    ...
    def validate_secret_key(cls, key: str) -> str:
        ...
        if len(key) > cls.MAX_KEY_LENGTH:
            raise ValueError(
                f"Secret key exceeds maximum length of {cls.MAX_KEY_LENGTH}"
            )
    ...
    def validate_threshold(cls, threshold: int, num_keys: int) -> int:
        ...
        if threshold > num_keys:
            raise ValueError(
                f"Threshold ({threshold}) cannot exceed number of keys ({num_keys})"
            )
    ...
    def validate_spec(cls, spec: Dict[str, Any]) -> Dict[str, Any]:
        ...
        if not re.match(r"^\d+[smh]$", interval):
            raise ValueError(
                "reconcileInterval must be in format like '30s', '5m', '1h'"
            )
            ...
            if "matchLabels" in pod_selector:
                for key, value in pod_selector["matchLabels"].items():
                    if not isinstance(key, str) or not isinstance(value, str):
                        raise ValueError(
                            "podSelector.matchLabels keys and values must be strings"
                        )
