...
class VaultClient:
        ...
        try:
            ...
            for i, key in enumerate(validated_keys[:threshold]):
                try:
                    ...
                    try:
                        decoded_key = base64.b64decode(key, validate=True).decode('utf-8')
                    except Exception as e:
                    ...
                    import asyncio
                    loop = asyncio.get_event_loop()
                    result = await loop.run_in_executor(None, self.client.sys.submit_unseal_key, decoded_key)
                    keys_submitted += 1
                    ...
                    if not result.get('sealed', True):
                        logger.info(f"Successfully unsealed vault at {self.url} with {keys_submitted} keys")
                        return result
                        
                except Exception as e:
                    last_error = e
                    logger.error(f"Failed to submit unseal key {i+1} to {self.url}: {e}")
            ...
            sanitized_error = str(e).replace(str(keys), "[REDACTED]") if keys else str(e)
    ...
    async def health_check(self) -> Dict[str, Any]:
        try:
            import asyncio
            loop = asyncio.get_event_loop()
            health = await loop.run_in_executor(None, self.client.sys.read_health_status)
            return health
        except Exception as e:
