...
class VaultOperatorV2:
    ...
    async def get_unseal_keys(self, spec: Dict[str, Any], namespace: str) -> List[str]:
        ...
        if "secretRef" in unseal_keys_spec and unseal_keys_spec["secretRef"]:
            ...
            if not isinstance(secret_ref, dict) or "name" not in secret_ref:
                raise ValueError(
                    "unsealKeys.secretRef must be an object with 'name' field"
                )
            ...
            try:
                ...
                if key in secret.data:
                    ...
                    try:
                        encoded_keys = base64.b64decode(secret.data[key]).decode(
                            "utf-8"
                        )
                    ...
                    try:
                        ...
                        keys = [
                            k.strip() for k in encoded_keys.split("\n") if k.strip()
                        ]
                        if not keys:
    ...
    async def check_and_unseal_vault(
        self, spec: Dict[str, Any], namespace: str, name: str
    ) -> VaultStatus:
        ...
        try:
            ...
            return VaultStatus(
                sealed=is_sealed,
                lastUnsealed=(
                    datetime.now(timezone.utc).isoformat() if not is_sealed else None
                ),
            ...
            "conditions": [
                {
                    ...
                    "reason": (
                        "ConfigurationApplied" if not vault_status.error else "Error"
                    ),
                    "message": (
                        "Vault configuration applied successfully"
                        if not vault_status.error
                        else vault_status.error
                    ),
                }
            ],
            "vaultStatus": {
